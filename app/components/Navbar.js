    ...
    <>
      ...
      >
        <div className="w-full flex justify-between items-center gap-4">
          ...
          <button
            className="flex md:hidden justify-center items-center gap-2 py-2 px-6 border rounded-full  border-zprimary-800 text-sm"
      ...
      <div
        className={`${
          NavState ? "translate-y-[100vh]" : "translate-y-0"
        } fixed bottom-0 transition ease-in-out delay-150  z-50  w-full h-screen flex md:hidden justify-center items-end `}
        onClick={() => setNavState(!NavState)}
      >
        <div className="p-12 backdrop-brightness-50 backdrop-blur-[140px] w-[90vw] rounded-t-3xl shadow-[inset_0_0px_80px_0px_var(--zprimary-900)]">
          <ul className="grid grid-cols-3 gap-4 text-sm md:text-sm w-full">
            <li>
              <Link
                href="/"
                className={
                  pathname == "/"
                    ? "text-sm text-zprimary-400 font-base font-xs flex flex-col justify-center items-center aspect-square"
                    : "text-sm text-zgary2 font-base font-xs flex flex-col justify-center items-center aspect-square"
                }
              >
                <GoHomeFill
                  className={
                    pathname == "/"
                      ? "text-6xl border-2 bg-zprimary-500/20 p-4 border-zprimary-800 shadow-[0_0px_40px_0px_var(--zprimary-900)] rounded-xl"
                      : "text-6xl border-2 bg-transparent p-4 border-gray-700 rounded-xl"
                  }
                />
                Home
              </Link>
